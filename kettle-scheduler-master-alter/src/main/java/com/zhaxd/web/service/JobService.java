...
//		Long allCount = kJobDao.templateCount(template);
     ...
     * @return void
     * @Title startT
     * @Description 服务启动时自动执行的方法 ， 负责启动应该启动的作业
     */
    ...
    private void initStr(){
        ...
        if (kettleLogFilePath==null||"".equals(kettleLogFilePath)){
            ...
            kJobDao = sqlManager.getMapper(KJobDao.class);
            kJobMonitorDao = sqlManager.getMapper(KJobMonitorDao.class);
        ...
        try {
            ...
            while (i <= maxI) {
                ...
                try {
                    ...
                    System.out.println("Job的自动执行 运行了" + i + "次");
                    if(flag){
                        return;
                    }
                    i++;
